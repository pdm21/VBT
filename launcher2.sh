...
# ────────────────────────────────────────────────────────────────
# 🆕 Start FastAPI Python backend server
cd "$PROJECT_DIR/python_backend" || {
    osascript -e 'display notification "Could not find python_backend directory" with title "VBT Launcher Error"'
    exit 1
}
...
# Launch FastAPI with logging
uvicorn main:app --host 0.0.0.0 --port 8000 > "$PROJECT_DIR/logs/python_backend.log" 2>&1 &
PY_BACKEND_PID=$!
