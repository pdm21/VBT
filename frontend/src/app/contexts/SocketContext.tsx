...
export function SocketProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!socketRef.current) {
      ...
      const socketInstance = io(`http://${process.env.NEXT_PUBLIC_SERVER_IP || '192.168.1.10'}:3001`, {
        reconnection: true,
        ...
        query: {
          ...
          deviceType: navigator.platform.toLowerCase().includes('mac') ? 'host' : 'client'
        }
      });
      ...
      socketInstance.on('connect', () => {
        ...
        setDeviceType(navigator.platform.toLowerCase().includes('mac') ? 'host' : 'client');
      });
      ...
      socketInstance.on('sessionState', (state: any) => {
        ...
        if (state.page === 'dashboard') {
          router.push(`/dashboard?${new URLSearchParams(state.params).toString()}`);
